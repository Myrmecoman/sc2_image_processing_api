...
# source : https://github.com/tawsifkamal/Deep-Q-Learning-CartPole-v0/blob/main/CartPoleDQL.py
...
policy_net = Network(bot.state.shape[0], bot.nb_actions).to(device) # state length, number possible actions
target_net = Network(bot.state.shape[0], bot.nb_actions).to(device)
...
for episode in range(1000):
    ...
    while not done:
        ...
        if env.bot.time - env.bot.timer < 1:
            continue
